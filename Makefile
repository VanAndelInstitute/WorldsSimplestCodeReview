...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "WorldsSimplestCodeReview", "tests")); system.time(test_check("WorldsSimplestCodeReview", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran WorldsSimplestCodeReview_$(VERSION).tar.gz
	rm -rf WorldsSimplestCodeReview.Rcheck/
